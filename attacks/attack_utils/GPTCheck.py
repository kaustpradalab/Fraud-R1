...
class GPTCheck:
    ...
    def process_prompt(self, language):
        if language == "Chinese":
            ...
            请严格输出JSON格式且仅包含指定选项（无需任何说明）：
            {"detection": "YES|NO|NEXT ROUND"}
            ...
            Please strictly output in JSON format and only include the specified options (no additional explanation needed):
            {"detection": "YES|NO|NEXT ROUND"}
    ...
    def judge(self, input, language):
        ...
        try:
            return json.loads(response_content)  # 尝试解析 JSON
        except json.JSONDecodeError:
            return response_content  # 解析失败时返回原始字符串
    """
